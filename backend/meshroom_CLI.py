...
# def meshmain(path):
#     binpath="C:\\Users\\codec\\Desktop\\Genemate\\Meshroom-2023.3.0\\aliceVision\\bin"
#     outpatyh="output"
#     # Pass the arguments of the function as parameters in the command line code
#     binPath =  binpath          ##  --> path of the binary files from Meshroom
#     baseDir = outpatyh           ##  --> name of the Folder containing the process (a new folder will be created)
#     imgDir = path            ##  --> Folder containing the images 
...
def meshmain(path):
    ...
    # Pass the arguments of the function as parameters in the command line code
    binPath = binpath          ##  --> path of the binary files from Meshroom
    baseDir = outpath          ##  --> name of the Folder containing the process (a new folder will be created)
    imgDir = path              ##  --> Folder containing the images 
    print(imgDir) 
    numberOfImages = len([name for name in os.listdir(imgDir) if os.path.isfile(os.path.join(imgDir, name))])      ## number of files in the folder
